...
//    }
    ...
    public List<TourApiDto> fetch(String url){
        ...
        try {
            ...
            for(Object o :jsonItemList){
                ...
                TourApiDto tourApiDto = makeLocationDto(item);
     ...
     * @author lsh
     */
    private TourApiDto makeLocationDto(JSONObject item) {

        return TourApiDto.builder()
                .contentId((String) item.get("contentid"))
                .title((String) item.get("title"))
                .address((String) item.get("addr1"))
                .areaCode(Long.parseLong((String) item.get("areacode"))) // 문자열을 Long으로 변환
